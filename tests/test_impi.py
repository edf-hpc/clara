...
def fake_popen(cmd = None, stdout=None, stderr=None):
    if "-e!" in cmd:
        return Popen('echo \"Chassis Power is on\"', stdout=PIPE, shell=True)

    return Popen('python tests/data/bin/impi', stdout=PIPE, shell=True)
...
def test_do_coonect(mocker):
    ...
    m_logging = mocker.patch("clara.plugins.clara_build.logging.warning")
    do_connect('host1')
...
def test_getmac(mocker):
    mocker.patch("clara.utils.getconfig", side_effect=fake_config)
    mocker.patch("clara.plugins.clara_ipmi.value_from_file", side_effect=fake_value)
